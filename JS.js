...
let sections = ["Rs. 25", "Rs. 15", "Rs. 50", "Rs. 1000", "Rs. 25",
                "Rs. 500", "Rs. 15", "Rs. 100", "Rs. 1000", "Rs. 15",
                "Rs. 100", "Rs. 500"];
...
function repaint(angle) {
    let r = Math.min(innerWidth, innerHeight) / 2.05 | 0;
    if (wheels === null) {
        wheels = [];
...
function prize(){
	alert("Congractulations !! You have won ");
}
...
canvas.onmousedown = function() {
    if (!running) {
	spinTo(Math.random()*sections.length|0, 3982);
	myPlay();
    }
};
repaint(angle);
